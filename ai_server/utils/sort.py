...
class BoxTracker():
    def __init__(self, img_arr, u_id, conf, left, top, width, height, center_x, center_y, class_id, src_id, frame_number, config, child_obj, frame_cache, timestamp):
        self.n2d = False
    ...
    def update(self, img_arr, u_id, conf, left, top, width, height, center_x, center_y, class_id, src_id, frame_number, child_obj, timestamp):
        # print(f"======================{class_id}===========================")
        # if width > self.config.width & height > self.config.height & conf > self.config.conf:
        if 1:
            # print(f"======================{self.frame_cache[src_id][0][1]}===============")
    ...
    def alert_trigger(self, alert_time, stream_id):
        """告警触发时，从指定流ID的缓存中提取与alert_time最接近的帧"""
        t0 = time.time()
        closest_frame = None
        min_diff = float('inf')

        # 遍历缓存找到与alert_time最接近的帧
        if stream_id in self.frame_cache:
            for frame, timestamp in self.frame_cache[stream_id]:
                time_diff = abs(alert_time - timestamp)
                if time_diff < min_diff:
                    min_diff = time_diff
                    if min_diff < 100:  
                        closest_frame = frame
        print(f"==================min diff:{min_diff}====耗时:{time.time()-t0}")
    ...
    def send(self,save=False):
        ...
        img_train = self.alert_trigger(timestamp, self.src_id)
        img_arr = copy.deepcopy(img_train)
        ...
        try:
            ...
            event = Event(
                ...
                property="None", 
                task_id=TaskTypeEnum.ValueType(11), # 暂定
                pic_type=PictureType.ValueType(2)  # 暂定
            )
            ...
            if child_obj is not None:
                event1 = Event(
                    ...
                    trains=[train_img_bytes],
                    rois=[roi_img_bytes1], 
                    box=Box(x1=int(child_obj[2]), y1=int(child_obj[3]), x2=int(child_obj[2]+child_obj[4]), y2=int(child_obj[3]+child_obj[5])), 
                    property="None", 
                    task_id=TaskTypeEnum.ValueType(11), # 暂定
                    pic_type=PictureType.ValueType(1)  # 暂定
                )
                # self.stub.PublishEvent(event1)
